...
- **Estrutura de pastas por marca/empresa**: cada pasta em `.claude/skills/<nome>/` representa um cliente/marca. Arquivos soltos direto na pasta = conteúdo novo pra postar nos canais daquele cliente (definidos no `SKILL.md` da pasta). Depois de publicado, o arquivo é movido pra `processados/` dentro da mesma pasta. **Regra fixa: só publica o que Franklin colocou direto na pasta daquele cliente — nunca mover/reclassificar um arquivo de uma pasta de skill pra outra como "correção".** Se um arquivo parecer ser de outra marca/empresa, a skill deve parar e reportar, não mover nem publicar (incidente real em 2026-08-20: um flyer de terceiro foi movido pra `kleber-construcao/` por engano e publicado nas redes do Kleber).
  ...
  - `kleber-construcao/` — empresa de construção (cliente). Skill completa. Canais conectados no Postiz desde 2026-08-20: Facebook, Instagram, TikTok Business. Plano do Kleber no app: **Especialista + Gestor de Tráfego** (slug `especialista`, não Personalizado — ver correção em 2026-08-26 na seção de planos abaixo).
  - Outras empresas: ainda não criadas.

**Usuários reais do sistema hoje (confirmado por Franklin em 2026-08-26)**: só dois — Franklin como administrador total (login legado `frank`, senha mestra) e o Kleber, primeiro (e único) cliente cadastrado, plano Especialista + Gestor de Tráfego. Kleber já tem cadastro/login funcionando; qualquer liberação de acesso adicional pra ele continua sendo feita por Franklin direto no app, pelo próprio celular (Método 1 — ver seção "Métodos de acesso do cliente ao app"), não pelo Claude.
...
Franklin (`frank/`) está classificado como plano Personalizado — continua usando vídeo/design via Postiz/Veo3, o caminho de melhor qualidade.

**Correção em 2026-08-26**: o Kleber (`kleber-construcao/`) **não** é Personalizado — Franklin confirmou que o plano dele é **Especialista em Social Media + Gestor de Tráfego** (slug `especialista`, R$300). Isso muda o texto acima: o Kleber é, na prática, o primeiro cliente real de um dos planos Iniciante/Profissional/Especialista — então, pela regra de roteamento logo acima, a geração de foto/vídeo pra ele passaria a ser pelo caminho barato (`gestor-de-geracao-ia-google`), não mais Postiz/Veo3. **Ainda não migrado de fato** — a skill `kleber-construcao/` (ver seu `SKILL.md`) continua publicando via Postiz normalmente (isso não muda, é só o passo de publicação); o que mudaria é de onde vem o conteúdo gerado antes de cair na pasta da skill. Confirmar com Franklin antes de tratar essa migração de pipeline como decidida.
