...
**Relatório administrativo (2026-08-25, revisado no mesmo dia)**: dentro da mesma caixa "Liberar cliente", um botão "Relatório administrativo" mostra **inline, na mesma página** (não é mais link pra página separada — Franklin pediu pra reaproveitar o login já feito, sem pedir a senha mestra de novo) **quantas contas estão cadastradas** e **quantas já logaram alguma vez**, com a lista (nome, e-mail/telefone, plano, último login). Reaproveita `session.password` (que, pra quem logou como `frank`, já é a própria senha mestra) — chama `/api/admin-report` sem pedir nada digitado de novo. Endpoint nunca devolve `passwordHash`. Pra sustentar o "já logaram", `app/api/_lib/auth.js` grava `user.lastLogin` a cada login bem-sucedido de cliente comum (login legado da senha mestra não conta, não é uma conta de cliente). **Não existe mais `app/public/relatorio.html`** (página separada removida — mesma lição do "Liberar cliente": nada que exija logar nem digitar senha de novo, tudo inline na sessão já aberta). **Franklin vai pedir mais campos de relatório aos poucos — ir adicionando nesse mesmo bloco/endpoint conforme ele pedir, não inventar métricas novas sem ele pedir.**

**Relatório das redes sociais — pra TODO cliente logado, inclusive o Franklin/`frank`, não só admin (2026-08-25)**: botão "📊 Relatório das redes sociais" dentro do próprio painel logado (`#social-report`, **posicionado embaixo do botão "Enviar"**, junto com a caixa "Liberar cliente" — mesmo padrão de posição já usado pra ela), visível pra qualquer cliente logado, sem exceção — inclusive quando o Franklin loga como `frank`, ele também vê e usa esse mesmo botão (só que com o bloco extra abaixo). Ao clicar, reaproveita `session.identifier`/`session.password` já capturados no login (não pede senha de novo) e chama `/api/social-report.js`, que mostra: plano do cliente e quais redes sociais ele já conectou (`user.connections`). **Quando quem loga é o `frank`, o mesmo botão mostra um bloco extra**: quantidade total de clientes cadastrados (reaproveita `loadUsers().length`) — é a "informação a mais" que só o admin vê, mesmo botão pra todo mundo.

**Visual do botão (2026-08-25)**: usa a mesma classe `social-networks-title` do botão "Escolha aqui as suas redes sociais" (pílula laranja/amarela com borda vermelha, texto branco maiúsculo, brilho pulsante) — pedido explícito do Franklin de reaproveitar exatamente esse modelo visual, não um botão novo/diferente.

**Pendência clara, não fingir que já existe**: hoje esse relatório mostra só quais redes estão conectadas, **não métricas de desempenho de verdade** (curtidas, alcance, engajamento, etc.) — isso exige integrar com a API de Insights de cada rede (Meta Graph API Insights, TikTok Analytics), que é trabalho separado, ainda não feito. Avisar o Franklin disso se ele perguntar por números de engajamento — não inventar números.
