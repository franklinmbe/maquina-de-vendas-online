...
### Liberar cliente novo pra cadastro — fluxo definitivo (fixado em 2026-08-25)

**Como funciona, do jeito que Franklin decidiu depois de testar as alternativas**: Franklin manda pro Claude, aqui no chat, o e-mail do cliente + a senha que ele quer pra essa conta + o plano contratado. O Claude cria a conta direto por trás (chamando `/api/admin-set-account`, ver abaixo) — **sem nenhuma tela nova, sem página própria, sem o cliente nem o Franklin precisar preencher formulário nenhum**. O cliente só recebe e-mail/senha do Franklin por fora (WhatsApp, etc.) e entra direto na aba "Entrar" normal do app, que já existe. Isso vale como **padrão daqui pra frente**, não é mais exceção pontual — Franklin pediu isso explicitamente, várias vezes, depois de a alternativa (autocadastro / formulário de liberação) causar mais trabalho do que resolver.

**Limitação importante**: esse fluxo exige uma sessão do Claude com acesso real à internet pra chamar o endpoint. **Esta sessão remota específica tem o egress bloqueado pro domínio do site** (`maquina-de-vendas-online-five.vercel.app`) — então nela o Claude só consegue preparar/documentar a chamada, não executá-la de fato. Numa sessão local (PC do Franklin) isso deve funcionar normalmente. Se uma sessão tentar isso e não conseguir alcançar o domínio, avisar o Franklin em vez de fingir que funcionou.

**`/api/admin-set-account`** (`app/api/admin-set-account.js`) — `POST` com `{ passphrase, name, identifier, client, plan, altIdentifier, password }`. `passphrase` precisa bater com `APP_PASSPHRASE` (a senha mestra do Franklin). Cria ou substitui a conta já com a senha definida, pulando o autocadastro. Sem página própria — só o endpoint.

**Tentativa anterior revertida no mesmo dia**: chegou a existir uma página `liberar.html` (com e sem simplificação) pra Franklin preencher um formulário pelo celular. Levou ~2h de ida e volta pra liberar 1 único cliente (Kleber), mesmo já simplificada — Franklin pediu pra apagar tudo e decidiu pelo fluxo acima em vez disso. **Não recriar nenhuma página pra isso sem o Franklin pedir de novo.**

**A senha do cliente passa pelo chat com o Claude nesse fluxo, por decisão explícita e repetida do Franklin como dono do produto** — não é o comportamento que o Claude teria escolhido por padrão (ver histórico do dia), mas é a decisão final dele depois de testar a alternativa. `SIGNUP_ALLOWLIST` no Vercel (o autocadastro tradicional, sem senha passando pelo Claude) continua existindo em `app/api/register.js` como caminho alternativo, mas não é mais o padrão usado no dia a dia.
