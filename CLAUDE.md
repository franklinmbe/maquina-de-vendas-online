...
**Relatório administrativo (`app/public/relatorio.html`, 2026-08-25)**: dentro da mesma caixa "Liberar cliente", um link "Relatório administrativo" abre uma página separada (essa sim, página própria — pedido explícito do Franklin, diferente do "Liberar cliente" que ele não quis como página) pra ver **quantas contas estão cadastradas** e **quantas já logaram alguma vez**, com a lista (nome, e-mail/telefone, plano, último login). Pede a senha mestra na hora de carregar. Endpoint: `/api/admin-report` (`POST { passphrase }`, nunca devolve `passwordHash`). Pra sustentar o "já logaram", `app/api/_lib/auth.js` agora grava `user.lastLogin` a cada login bem-sucedido de cliente comum (login legado da senha mestra não conta, não é uma conta de cliente). **Franklin vai pedir mais campos de relatório aos poucos — ir adicionando nessa mesma página/endpoint conforme ele pedir, não inventar métricas novas sem ele pedir.**
