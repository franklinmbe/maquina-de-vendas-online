...
**Relatório das redes sociais — pra TODO cliente logado, não só admin (2026-08-25)**: botão "📊 Relatório das redes sociais" dentro do próprio painel logado (`#social-report`, topo de `#upload-fields`), visível pra qualquer cliente, não só `frank`. Ao clicar, reaproveita `session.identifier`/`session.password` já capturados no login (não pede senha de novo) e chama `/api/social-report.js`, que mostra: plano do cliente e quais redes sociais ele já conectou (`user.connections`). **Quando quem loga é o `frank`, o mesmo botão mostra um bloco extra**: quantidade total de clientes cadastrados (reaproveita `loadUsers().length`) — é a "informação a mais" que só o admin vê, mesmo botão pra todo mundo.
