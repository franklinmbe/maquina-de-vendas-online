...
- **Crescimento de seguidores/curtidas ao longo do tempo (2026-08-25)**: como o projeto não tem cron configurado, o histórico é gravado **sob demanda** — toda vez que o relatório é aberto, grava um retrato do dia em `user.growthHistory` (1 por dia, não duplica). Cresce mais devagar que uma coleta diária de verdade (só anda quando alguém abre o relatório), mas é histórico real, nunca inventado — o gráfico só aparece depois de existir mais de 1 dia de retrato. Se o Franklin quiser um histórico que cresça sozinho todo dia (sem depender de alguém abrir a página), isso é uma tarefa nova: configurar um cron de verdade no Vercel.
