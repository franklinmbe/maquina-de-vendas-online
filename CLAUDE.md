...
- **Aconteceu de novo em 2026-08-25**: mesmo já no Pro, uma sessão fez ~15 merges seguidos num único dia (um por ajuste pequeno, ignorando a lição acima) e bateu no limite de build de novo — `Vercel: Deployment rate limited — retry in 24 hours`. Como consequência, várias mudanças reais (destaque das estratégias, métricas extras do Meta, correção do bug do cron) ficaram mescladas no GitHub mas **não foram ao ar**, e ninguém percebeu na hora porque nenhuma sessão conferia o status do deploy depois de mesclar — só assumia que tinha ido pro ar. **A partir de agora: sempre conferir `pull_request_read` método `get_status` depois de mesclar, antes de dizer pro Franklin que algo "já está no ar"** — e voltar a agrupar várias mudanças pequenas num só PR/merge em vez de uma por ajuste.

### Pendências pra retomar assim que o Vercel voltar ao normal (salvo por pedido do Franklin em 2026-08-25)

Franklin pediu explicitamente pra deixar isso registrado como tarefa automática — ele não vai lembrar de pedir de novo, então a próxima sessão que mexer neste repo deve **conferir isto primeiro, sem precisar ser solicitada**, antes de considerar essa lista concluída:

1. **Confirmar que o deploy voltou a funcionar** — rodar `pull_request_read` método `get_status` no PR mais recente (ou no commit de topo da `main`) antes de mexer em qualquer coisa. Se ainda aparecer `Deployment rate limited`, esperar o reset (o limite é por 24h a partir de cada tentativa bloqueada) e não tentar mesclar nada novo enquanto isso — cada merge novo durante o rate limit só reinicia a espera.
2. **Assim que aparecer sucesso de deploy**, verificar ao vivo (pedindo pro Franklin abrir o app, já que sessões remotas não alcançam o domínio do site) se estas features — todas já mescladas na `main` via PR #99 a #104, mas nunca confirmadas no ar — estão realmente funcionando:
   - Destaque visual (pílula laranja/vermelha piscando) da linha de "Estratégias" no card do plano Personalizado, em `app/public/index.html` (classe `.plan-strategies-list`).
   - Botão "📊 Relatório das redes sociais" abrindo `relatorio-redes.html` em nova aba, autenticado via `mvo_report_handoff`.
   - Métricas do Meta no relatório: impressões, alcance, engajamento, visitas ao perfil, seguidores/fãs, top posts com comentários/curtidas.
   - Métricas extras do Meta (reações por tipo, alcance único, visitas à página, cliques de contato do Instagram) — essas ainda **nunca foram validadas contra uma conta real** (a sessão que implementou não tinha acesso de rede pra testar), então além de confirmar que aparecem, checar se os números batem com o que o Franklin vê no Meta Business Suite.
   - Histórico de crescimento de seguidores/curtidas (`user.growthHistory`) — hoje só é gravado **sob demanda**, quando alguém abre o relatório (o cron diário foi removido do `vercel.json` por suspeita de estar causando a falha de build — ver item 3). Sem o cron, o gráfico de crescimento só vai ter pontos nos dias em que o relatório for aberto manualmente — avisar o Franklin disso caso ele pergunte por que o gráfico está com poucos pontos.
