...
**Padrão em ambos os casos acima**: a allowlist libera só a *identidade* (quem pode se cadastrar, com qual plano) — nunca uma senha. O cliente cria e confirma a própria senha na tela de cadastro do app.

**Exceção — `/api/admin-set-account`, também acessível pela mesma página `liberar.html`** (caixinha "já criar a conta com uma senha", 2026-08-25, caso de origem: Kleber): cria a conta **já com uma senha definida**, pulando o autocadastro — pra quando Franklin administra a conta em conjunto com o cliente e precisa que ela já exista pronta pra logar. Só existe porque Franklin pediu explicitamente essa exceção pro Kleber, repetindo o pedido depois de o Claude explicar o padrão acima — decisão dele como dono do próprio produto/cliente, não o comportamento default. **A senha em si não deve ser colada no chat com o Claude** — o ponto desse formulário é justamente o próprio Franklin digitar a senha ali, na hora, sem ela passar pela conversa. Continua exigindo a senha mestra (`APP_PASSPHRASE`). Usar só quando pedido explicitamente pra um cliente específico — o padrão pra clientes novos continua sendo o autocadastro.
