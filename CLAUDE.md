...
- **Crescimento de seguidores/curtidas ao longo do tempo — coleta automática diária (2026-08-25)**: `app/api/cron/collect-social-snapshots.js`, disparado por cron de verdade do Vercel (`vercel.json`, `crons`, todo dia às 6h UTC = 3h em Brasília) — roda sozinho, sem depender de ninguém abrir o relatório. Grava o mesmo retrato diário (`user.growthHistory`, via `recordGrowthSnapshot` em `_lib/users.js`, compartilhado com `social-insights.js`) pra todo cliente nos planos com insights que tiver Meta conectado. O gráfico de crescimento em `relatorio-redes.html` só aparece depois de existir mais de 1 dia de retrato acumulado — nada retroativo, começa a contar a partir de quando essa coleta foi ligada.
  - **Pendência do Franklin**: definir `CRON_SECRET` (qualquer string aleatória) nas variáveis de ambiente do Vercel — o endpoint confere esse valor pra ninguém de fora poder disparar a coleta só sabendo a URL. Sem essa variável configurada, o endpoint funciona sem checar nada (não travar a coleta enquanto isso não for feito, mas recomendar configurar).
  - Cron do Vercel exige plano Hobby ou Pro (o projeto já é Pro) — 1x por dia está dentro do limite de qualquer um dos dois.
