...
- **"Relatório administrativo"** — dentro da caixa "Liberar cliente" (só `frank` vê). Abre `app/public/relatorio.html` numa aba nova, já carregado: total de contas cadastradas, quantas já logaram alguma vez, lista (nome, e-mail/telefone, plano, último login). Endpoint `/api/admin-report` (nunca devolve `passwordHash`).
...
**Ainda não existe** (pendência avisada, não inventar números): métricas reais de desempenho de rede social (curtidas, alcance, engajamento) — isso exige integrar com a API de Insights de cada rede (Meta Graph API, TikTok Analytics), trabalho separado, não feito ainda.
