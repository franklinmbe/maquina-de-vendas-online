...
- **Migração em andamento (decidida por Franklin em 2026-08-27)**: nova hospedagem na **Hostinger**, domínio registrado na **HostNet** (`app.franklinmorais.com`, subdomínio escolhido de propósito pra não mexer no site raiz `franklinmorais.com`, que é o WordPress do Franklin). Testado de ponta a ponta em 2026-08-26 (ver checklist abaixo). **Esta sessão remota não consegue acessar nem verificar o domínio ao vivo** (egress de rede bloqueado pra domínios externos — testado e confirmado), então qualquer checagem "está no ar agora?" depende do próprio Franklin ou de outra sessão de Claude Code rodando localmente no PC dele.
  - **Detalhe técnico da hospedagem (confirmado por sessão local em 2026-08-27)**: não é hospedagem compartilhada nem o produto "Hostinger Node.js hosting" — é uma **VPS Hostinger** (plano KVM1, Ubuntu 24.04), DNS da HostNet com registro A apontando pro IP da VPS (`179.199.132.109`), rodando um container Docker (`mvo`) com firewall (`ufw`) liberando só as portas 22/80/443/3000, e Nginx como reverse proxy pra `localhost:3000`. SSL via Certbot/Let's Encrypt com renovação automática, validado funcionando em 2026-08-26.
  - **Backend é código paralelo, não as mesmas funções da Vercel**: as rotas serverless `app/api/*.js` (formato Vercel) não rodam como estão na VPS. Existe um servidor separado, **`app/hostnet-server/`**, que reimplementa a mesma lógica só que como processo Node persistente — é esse que roda dentro do container Docker. **Esse código hoje só existe na branch `claude/project-access-9jn1o7`, ainda não mesclado na `main`.** Enquanto os dois backends existirem em paralelo (Vercel `app/api/*.js` e Hostinger `app/hostnet-server/`), qualquer funcionalidade nova ou correção precisa ser replicada nos dois lugares manualmente, senão o ambiente de produção real (Hostinger) fica desatualizado em relação ao que está documentado/testado só do lado Vercel.
  - **Deploy é manual, sem CI/CD**: editar em worktree local → commit → push pro branch `claude/project-access-9jn1o7` → SSH na VPS → `git pull`/`git reset --hard` → rebuild do container Docker. Diferente da Vercel, não publica sozinho a partir de um push.
  - **Variáveis de ambiente**: ficam num arquivo `.env` direto na VPS (`app/hostnet-server/.env`), não num painel de env vars da Hostinger. Confirmados com valor real: `GITHUB_TOKEN`, `GITHUB_OWNER`, `GITHUB_REPO`, `APP_PASSPHRASE`, `OAUTH_STATE_SECRET`, `TOKEN_ENCRYPTION_KEY`, `META_APP_ID`, `META_APP_SECRET`, `META_LOGIN_CONFIG_ID`, `TIKTOK_CLIENT_KEY`, `TIKTOK_CLIENT_SECRET`, `YOUTUBE_CLIENT_ID`, `YOUTUBE_CLIENT_SECRET`, `TELEGRAM_BOT_TOKEN`. Ainda sem valor real: `SIGNUP_ALLOWLIST` (autocadastro real ainda não configurado nesse servidor); `BLOB_READ_WRITE_TOKEN`/`USERS_BLOB_SECRET` nem se aplicam aqui (eram só da Vercel Blob). **Não confirmado se `POSTIZ_API_KEY` está configurada nesse `.env`** — conferir direto antes de assumir que publicação via Postiz funciona a partir desse servidor.
  - **Testado e funcionando de ponta a ponta (2026-08-26/27)**: login (senha mestra e conta real) + cadastro + envio de pedido com foto (confirma chegada em `.claude/skills/<cliente>/` no GitHub `main`); Meta (Facebook + Instagram) OAuth connect; TikTok OAuth connect; YouTube OAuth + upload de vídeo real; WordPress (ciclo completo: criar post, subir mídia, deletar) contra o site real `franklinmorais.com`; Telegram (bot como admin de canal), validado ao vivo em 2026-08-27.
  ...
  - **Condição pra cancelar a Vercel**: só cancelar definitivamente **depois de confirmar que a Hostinger está funcionando corretamente por um tempo** — Franklin estimou algo em torno de 10 dias a partir de 2026-08-27 antes de decidir cancelar. Não cancelar antes disso, e não fazer isso sem ele pedir explicitamente.
  - **Risco pendente, não resolvido**: os cadastros reais de clientes hoje vivem no **Vercel Blob** (`users.json`); ainda não foram migrados pro `users.json`/`DATA_DIR` da VPS Hostinger. Trocar o domínio de produção final pra Hostinger **antes** dessa migração faria todo cliente já cadastrado perder login/dados. Ver `app/hostnet-server/DEPLOY.md` (branch `claude/project-access-9jn1o7`) pro passo a passo de migração quando for a hora.
  - **Outras pendências explícitas, não fazer sem Franklin pedir**: `SIGNUP_ALLOWLIST` real pro autocadastro de clientes nesse servidor; Ads/Analytics/Google Meu Negócio (nem começados).
