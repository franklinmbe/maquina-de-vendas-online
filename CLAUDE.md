...
**Pendências antes de considerar essa VPS pronta pra receber cadastro de cliente real**:
1. `APP_PASSPHRASE` — hoje com valor temporário no servidor novo; trocar pela senha real de Franklin antes de liberar cliente de verdade por lá.
2. `SIGNUP_ALLOWLIST` real — vazia/inválida hoje; só importa quando for testar/liberar autocadastro de cliente (Método 2) nesse servidor.
3. **Migrar os cadastros de cliente já existentes**: baixar o `users.json` do Vercel Blob e colocar no volume `DATA_DIR` do servidor novo — sem isso, todo cliente já cadastrado no Vercel perde login/dados quando o domínio final apontar pra Hostinger. Ver `app/hostnet-server/DEPLOY.md` pro passo a passo. **Nenhum cliente real deve ser cadastrado direto no servidor novo antes dessa migração**, pra não ter dois `users.json` divergentes (um na Vercel, outro na VPS) — isso duplicaria/perderia cadastros dependendo de qual for copiado por cima do outro depois.
4. Atualizar a Redirect URI no Meta for Developers e no TikTok for Developers pro domínio novo (`app.franklinmorais.com`) — sem isso, conectar rede social quebra a partir desse servidor.
5. Confirmar se `POSTIZ_API_KEY` está configurada no `.env` da VPS (ver achado acima).
6. Depois de 1–5 resolvidos: validar de novo o ciclo completo (login, cadastro, pedido, conectar uma rede social de teste) direto no domínio final antes de considerar "pronto pra cliente real".
