...
- **Sempre escrever `legenda.txt`** na raiz da pasta com uma legenda curta pronta pra publicação (poucas frases, tom comercial, nunca re-listando perguntas de esclarecimento que ficaram sem resposta) — nunca deixar essa etapa condicionada a `videoAnalysis` existir. Bug real 2026-09-15: um pedido do Kleber sem vídeo pra analisar não gerou `legenda.txt`, caiu no fallback de `auto-publish.js` (que lê `instrucoes.txt` cru) e publicou ao vivo no Facebook/Instagram a pergunta não respondida do assistente do composer ("quer incluir preço/telefone? qual tom de voz?") como se fosse a legenda do post. Se `videoAnalysis` existir no status, basear a legenda na "LEGENDA SUGERIDA" da análise (mesmo mecanismo do passthrough, ver Passo 3); senão, compor a legenda a partir do pedido em `instrucoes.txt` (só a parte que descreve o que o cliente quer, ignorando qualquer troca de perguntas/respostas do assistente) e do que foi de fato gerado. (`auto-publish.js` também ganhou uma rede de segurança pro mesmo cenário — `extractCleanCaption` — mas não depender só dela: a legenda escrita aqui é sempre melhor que a extração automática.)
