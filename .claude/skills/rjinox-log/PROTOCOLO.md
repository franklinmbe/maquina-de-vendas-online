...
## Regra fixa: o conteúdo é SÓ da Rjinox — nunca nome, telefone nem imagem de vendedor (definida por Franklin em 2026-09-17, ampliada em 2026-09-21)

**Nenhum banner, foto, vídeo, legenda ou narração publicado pode trazer o nome do vendedor (Eduardo/Dudu, Jaqueline/Jack, Aline, Alessandra/Ale), o telefone dele, nem mostrar um vendedor** — nem como texto na imagem, nem na legenda, nem falado na narração, nem como pessoa/rosto de vendedor numa foto ou banner. Vale pros 4 vendedores. Foco: o produto, a cozinha/equipamento e a marca da empresa. Pra chamada à ação, frase genérica ("Fale com nosso time!"), nunca nome nem número.

O telefone e o nome continuam existindo e sendo usados por dentro (login no app, `log-publicacoes.md`, campanha de clique-para-WhatsApp no Meta Ads Manager — ver seções acima) — só não podem aparecer pro público no conteúdo em si.

**Aplicação no código (desde 2026-09-21)**: `app/hostnet-server/lib/client-content-rules.js`, pra qualquer cliente cujo slug termina em `-rjinox` — (1) o planejador da geração automática recebe a regra; (2) todo prompt de banner ganha um reforço fixo e a legenda/narração são limpas de nome/telefone antes de gerar; (3) a legenda final é limpa de novo na hora de publicar (aprovação e agendamento). **Leitura de tudo que está escrito/falado nas mídias (desde 2026-09-21, pedido do Franklin)**: `app/hostnet-server/lib/media-text-detection.js`. Em todo pedido de todo cliente o servidor lê o texto de cada imagem (OCR por visão) e usa a fala + texto de tela que a análise do vídeo já traz, e grava em `texto-detectado.json` na pasta do pedido. **Só pra Rjinox isso bloqueia**: foto/vídeo com nome ou telefone de vendedor **não vai pra revisão/publicação** (nem como está, nem como slide de vídeo); banner gerado pela IA é lido depois de pronto e refeito 1 vez se sair com nome/telefone (se persistir, é descartado); mídia que não deu pra ler também é barrada (falha fechada). O agendador (que publica sem passar pela geração) confere cada imagem/vídeo antes de publicar. O motivo aparece pro vendedor no acompanhamento do pedido (`vendorBlocks` / status `blocked_vendor_identifier` em `geracao-status.json`); se nada sobrar pra publicar, o pedido para com a explicação.
