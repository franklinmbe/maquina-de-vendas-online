---
name: jaqueline-rjinox
description: Verifica se chegou algum arquivo novo (imagem ou vídeo) na pasta .claude/skills/jaqueline-rjinox/ e, se sim, publica na página compartilhada da Rjinox (Facebook/Instagram/TikTok, via Postiz) — identificando sempre que o conteúdo é da vendedora Jaqueline/Jack (telefone 21994722099, login dela no app), pra depois virar anúncio no grupo de anúncios dela no Meta Ads.
---

Esta skill cuida do conteúdo da Jaqueline (Jack), uma das 4 vendedoras da Rjinox (não é marca própria do Franklin, nem um cliente único — a Rjinox tem 4 vendedores, cada um com pasta e telefone próprios: `eduardo-rjinox`, `jaqueline-rjinox`, `aline-rjinox`, `alessandra-rjinox`). Os arquivos ficam soltos direto nesta pasta (`.claude/skills/jaqueline-rjinox/`), fora de `processados/`.

**Ver `.claude/skills/rjinox-log/PROTOCOLO.md` pro protocolo completo compartilhado entre os 4 vendedores** (por que a separação existe, formato do log, IDs de integração da Postiz, regra de troca de anúncio) — esta skill só cobre os passos específicos de processar o conteúdo da Jaqueline.
...
3. **Nenhum dos dois**: pergunte à Jaqueline (via Franklin) do que se trata antes de continuar — não invente a legenda do zero sem nenhuma pista.
...
Depois de publicar com sucesso, adicione uma linha em `.claude/skills/rjinox-log/log-publicacoes.md`: data, **Jaqueline**, telefone **21994722099**, nome do arquivo, redes publicadas, e o link/ID do post retornado pela Postiz. Sem essa linha, a publicação não está completa.
...
- **Nunca mova um arquivo de outra pasta de skill (nem das outras vendedoras/vendedores da Rjinox) pra dentro de `jaqueline-rjinox/`, nem publique aqui algo que não foi colocado originalmente nesta pasta como conteúdo da Jaqueline.** Se algo parecer ser de outro vendedor, não mova nem publique — pare e reporte a dúvida pro Franklin decidir (mesma regra de sempre, ver incidente registrado em `.claude/skills/kleber-construcao/SKILL.md`).
