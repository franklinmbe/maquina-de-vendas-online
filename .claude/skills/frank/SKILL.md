...
- **Montagem de vídeo (fotos + música/narração)**: use `ffmpeg` via Bash. **Se o comando `ffmpeg` não for encontrado, instale primeiro com `apt-get update && apt-get install -y ffmpeg`** (roda como root neste ambiente, não precisa de `sudo`; confirmado funcionando 2026-08-19). Receita testada e aprovada pelo Franklin (usada em produção):
  ...
  - `filter_complex`: primeiro `scale`+`setsar`+`fps=30` (**fixar sempre em 30**, nunca deixar no padrão/implícito) de cada `[i:v]` pra uma resolução comum, depois encadear `xfade=transition=dissolve:duration={fade}:offset={k*D}` par a par — **o offset é cumulativo `k*D` (não `k*(D+fade)`)**, errar isso desincroniza do áudio.
  - Mapear a saída final do xfade + a faixa de áudio: `-c:v libx264 -pix_fmt yuv420p -r 30 -c:a aac -shortest`. **`-r 30` no output é obrigatório, não só o `fps=30` do filtro** — achado real 2026-09-15: um vídeo publicado sem essa flag de saída (só com `fps` no filtro) foi aceito por Facebook/Instagram mas **rejeitado pelo TikTok Business** (`Video frame rate is invalid, please check video specifications`) — o encadeamento de `xfade` pode deixar o vídeo com frame rate variável (VFR) mesmo com `fps=` no filtro de entrada, e o TikTok é mais rígido que Meta quanto a isso. `-r 30` no mapeamento final força frame rate constante (CFR) na saída.
